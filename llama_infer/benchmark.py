...
import numpy as np
import pandas as pd
from transformers import AutoTokenizer, GenerationConfig, LogitsProcessor, LogitsProcessorList, LlamaForCausalLM
...
def generate_torch(model, input_ids, n_generate):
    ...
    with torch.inference_mode():
        for i in range(n_generate):
            ...
            if i == 0:
                context_time += start.elapsed_time(end) * 1e-3
            else:
                generate_time.append(start.elapsed_time(end) * 1e-3)
    
    return context_time, generate_time

def run_round(generator, model_path, n_generate, input_ids, batch_size):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="meta/Llama-2-7b-chat-hf", help="path to the model")
